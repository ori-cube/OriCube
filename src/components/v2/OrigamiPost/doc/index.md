...
| 5-3      | 投稿データ化（答え形式＋ジェスチャ履歴のエクスポート）    | 🔄 予定         |
| 5-4      | v2 閲覧の再生                                             | 🔄 予定         |
...
- リプレイしながら各ステップの結果を答え形式へ書き出す純関数を追加
  - `FoldStep` → Base 相当（fixBoards / moveBoards / rotateAxis）
  - `SquashFoldStep` / `PetalFoldStep` → convolution 相当
    （movingPieces から nodes / moveNodesIdx / rotateAxes）
  - 注意: squash の `openRotate` は合成回転（折り線→ヒンジ）のため、
    旧 convolution の「頂点 1 軸」では表現できない。v2 答え形式は
    **頂点ごとに軸の列（1〜2 本）を順に適用**する形へ一般化する
- ジェスチャ履歴のシリアライズ（`THREE.Vector3` → `[x, y, z]`）を
  答え形式と一緒に保存し、既存 `POST /api/data` へ接続
