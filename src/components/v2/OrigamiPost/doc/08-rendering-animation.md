...
折り角がπ未満の `FoldStep`（`angle`、例: 鶴の羽の150度折り）で動いた板は、エンジン上は平面（180度折り）の位置のまま、描画時だけ折り線周りに `(angle − π)` 回転した姿勢で表示する。回転情報は `replayFoldStepsDetailed` が `FinishingRotation`（基準点・軸・角度）として収集し、軸はアニメーションと同じ `determineFoldRotation` で決めるため折り終わりの姿勢と表示が一致する。仕上げ角度の板が後続の折りの対象になると回転情報は破棄される（仕上げは最後の手として使う想定）。スナップポイントは平面プロキシの位置に出るため、仕上げ後の板の頂点は表示位置とずれる（既知の制限）。

### ビューモード — useViewMode

Toolbar のトグルで `viewing` フェーズへ入ると、ドラッグ&ドロップと Undo/Redo を止めて OrbitControls の回転を有効化し（通常はドラッグを折り操作に使うため無効）、折り上がりを自由な視点で確認できる。`useRenderBoards` は viewing ではスナップポイントを描画しない（板の表示は通常時と同じ）。終了時はカメラを開始時の位置へ戻す。
