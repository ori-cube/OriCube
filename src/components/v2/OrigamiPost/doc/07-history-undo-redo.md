...
interface FoldHistory {
  steps: OrigamiStep[]; // 折り操作の全履歴（通常の折り | 開いて畳む）
...
- **スナップショット不要**: 保存するのは `OrigamiStep`（折り線・頂点・枚数・視点）だけで、板の座標列は持たない
- **リプレイは決定的**: `OrigamiStep` はカメラ等の外部状態に依存しない（[02](./02-data-model.md)）

履歴要素は `kind` で判別するユニオンなので、開いて畳む（[09](./09-squash-fold.md)）も専用の巻き戻し処理なしで同じ仕組みに乗る。`replayFoldSteps` がリプレイ時に `kind` で適用関数を振り分けるだけ。
...
- 履歴には適用（`applyFoldStep` / `applySquashFoldStep`）が成功したステップだけが積まれる（`replayFoldSteps` がリプレイ中に null に遭遇した場合はそこで打ち切る防御的処理があるが、通常は起こらない）
